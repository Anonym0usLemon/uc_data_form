...
var alwaysInclude = ["80#", "100#", "105#", "130#", "UV Coat", "Leather", "Starline", "Velveteen", "Art Cloth", "Photowrap", "Laminate", "Clear Cover", "Frosted Cover", "Sticker", "Protective Seal", "Do not score", "Mounting", "Card Stock", "Metallic", "Texture", "Drymount included.", "Lustre coat included.", "Insert to folders.", "Laminate included.", "Sintra", "Bevel", "Envelopes", "Corners", "Wild Sorbet", "Grommet", "Custom Cut-out", "Rectangle/Square Cut", "Style", "Curve Direction", "Concave", "Convex", "Mocha Frame", "White Frame", "Black Frame", "Silver Frame", "White Base", "Natural Base", "8GB USB Drive - 1 Side Engraved", "8GB USB Drive - 2 Sides Engraved", "16GB USB Drive - 1 Side Engraved", "16GB USB Drive - 2 Sides Engraved", "Aquarelle Rag", "Arches Velin Museum Rag", "Baryta Photographique", "Rag Photographique", "ID Card H", "ID Card V", "14x17", "14x18", "4x5 Loose Proof - Blank", "4x5 Loose Proof - Bordered", "5x7 Loose Proof - Blank", "8.5x11 V Spiral Bound (Side) w/ Plastic Cover" ];
...
for (var i = 0; (item = items[i]) != null; i++) {
    ...
    var code = getElementsByClassName(item, "code")[0].innerHTML;
	if (code == "(61021000)")
	{itemName = "Single Unit";}
	if (code == "(61021001)")
	{itemName = "Single Unit";}
	if (code == "(61020500)")
	{itemName = "Half Unit";}
	if (code == "(20157025)")
	{itemName = "Legacy 5x7 Artistry Trifold #100 - Style 002";}	
	if (code == "(06000099)")
	{itemName = "5x7 Printed Linen Envelope";}
	if (code == "(99991000)")
	{itemName = "Stuff Envelopes";}
	...
	for (var j = 0; (option = options[j]) != null; j++) {
		...
		//Code to show embossing color in order services section - NR 3-13-2013
		if((optName.substring(optName.length - 9) == "Embossing")){
			OrderServices = document.getElementById("order_services");
			OrdServicesHTML = OrderServices.innerHTML + "<br>" + optName;
			OrderServices.innerHTML = OrdServicesHTML;
		}
...
if (document.getElementById('promo') && document.getElementById('promo_text') && document.getElementById('promo_text').innerHTML == "") document.getElementById('promo').style.display = "none";